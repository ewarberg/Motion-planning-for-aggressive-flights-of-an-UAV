...
class SingleAxisTrajectory:
    ...
    def generate(self, Tf):
        ...
        delta_a = self._af - self._a0  # eq. 24
        delta_v = self._vf - self._v0 - self._a0 * Tf  # eq. 24
        delta_p = self._pf - self._p0 - self._v0 * Tf - 0.5 * self._a0 * Tf * Tf  # eq. 24
        ...
        if self._posGoalDefined and self._velGoalDefined and self._accGoalDefined:
            self._a = (60 * T2 * delta_a - 360 * Tf * delta_v + 720 * 1 * delta_p) / T5  # eq. 25
            self._b = (-24 * T3 * delta_a + 168 * T2 * delta_v - 360 * Tf * delta_p) / T5  # eq. 25
            self._g = (3 * T4 * delta_a - 24 * T3 * delta_v + 60 * T2 * delta_p) / T5  # eq. 25
...
class RapidTrajectory:
    ...
    def set_goal_position(self, pos):
        ...
        for i in range(3):
            if pos[i] is None:
                continue
            self.set_goal_position_in_axis(i, pos[i])  # calls set_goal_position in SingleAxisTrajectory class
    ...
    def set_goal_velocity(self, vel):
        ...
        for i in range(3):
            if vel[i] is None:
                continue
            self.set_goal_velocity_in_axis(i, vel[i])  # calls set_goal_velocity in SingleAxisTrajectory class
    ...
    def set_goal_acceleration(self, acc):
        ...
        for i in range(3):
            if acc[i] is None:
                continue
            self.set_goal_acceleration_in_axis(i, acc[i])  # calls set_goal_acceleration in SingleAxisTrajectory class

    def set_goal_position_in_axis(self, axNum, pos):
        """ Define the goal end position in axis `axNum`."""
        self._axis[axNum].set_goal_position(pos)  # calls set_goal_position_in_axis in SingleAxisTrajectory class

    def set_goal_velocity_in_axis(self, axNum, vel):
        """ Define the goal end velocity in axis `axNum`."""
        self._axis[axNum].set_goal_velocity(vel)  # calls set_goal_velocity_in_axis in SingleAxisTrajectory class

    def set_goal_acceleration_in_axis(self, axNum, acc):
        """ Define the goal end acceleration in axis `axNum`."""
        self._axis[axNum].set_goal_acceleration(acc)  # calls set_goal_acceleration() in SingleAxisTrajectory class

    def set_goal_rotation_in_axis(self, axNum, rot):
        """ Define the goal end acceleration in axis `axNum`."""
        self._axis[axNum].set_goal_rotation(rot)
    ...
    def reset(self):
        ...
        """
        for i in range(3):
            self._axis[i].reset()  # calls reset() in SingleAxisTrajectory class
    ...
    def generate(self, timeToGo):
        ...
        self._tf = timeToGo
        for i in range(3):
            self._axis[i].generate(self._tf)  # calls generate() in SingleAxisTrajectory class
